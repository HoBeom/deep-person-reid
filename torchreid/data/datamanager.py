...
from torchreid.data.datasets import init_image_dataset, init_video_dataset, init_video_rl_dataset
...
class VideoRLDataManager(DataManager):
    r"""Video data manager.
    ...
    def __init__(
        ...
        super(VideoRLDataManager, self).__init__(
            sources=sources,
            targets=targets,
            height=height,
            width=width,
        ...
        transforms = Compose([ToTensor()]) # for not use transform

        print('=> Loading train (source) dataset')
        trainset = []
        for name in self.sources:
            trainset_ = init_video_rl_dataset(
                name,
        ...
        for name in self.targets:
            # build query loader
            queryset = init_video_rl_dataset(
                name,
                transform=transforms,
                mode='query',
                combineall=combineall,
            ...
            # build gallery loader
            galleryset = init_video_rl_dataset(
                name,
                transform=transforms,
                mode='gallery',
                combineall=combineall,
