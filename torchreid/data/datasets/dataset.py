...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.sample_method == 'random':
            ...
            if self.seq_len == 1:
                indices = np.array([num_imgs//2])
            elif num_imgs >= self.seq_len:
            ...
            # Samples the first images in a tracklet
            if num_imgs >= self.seq_len:
                indices = np.arange(0, self.seq_len)
            else:
                indices = np.arange(0, num_imgs)
                num_pads = self.seq_len - num_imgs
                indices = np.concatenate(
...
class VideoRLDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(VideoRLDataset, self).__init__(train, query, gallery, **kwargs)
        self.seq_len = seq_len
        self.sample_method = sample_method
    ...
    def __getitem__(self, index):
        ...
        if self.sample_method == 'random':
            ...
            if num_imgs >= self.seq_len:
                ...
                # interpolate the first image to the last image
                indices = np.arange(0, num_imgs)
                num_pads = self.seq_len - num_imgs
                indices = np.concatenate(
                    [
        ...
        for index in indices:
            ...
            try:
                ...
                raise RuntimeError('{} does not exist, run data preprocess'.format(feature_path))
            feature = torch.from_numpy(feature).float()
            features.append(feature)
        features = torch.cat(features, dim=0)

        item = {'feature': features, 'pid': pid, 'camid': camid, 'dsetid': dsetid}
